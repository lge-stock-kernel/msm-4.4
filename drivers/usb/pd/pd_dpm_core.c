...
#if defined(CONFIG_LGE_USB) && defined(CONFIG_DUAL_ROLE_USB_INTF)
...
#ifdef CONFIG_LGE_USB
	/*
	   For Ellisys PD compliance - TD.PD.VNDI.E1 VDM Identity
	   FAILED Sending Disocver Identity - UUT must respond with NAK
	 */
	if (pd_event->pd_msg->frame_type == TCPC_TX_SOP) {
		DPM_DBG("%s: reply NAK VDM Identity SOP packet\n", __func__);
		dpm_vdm_reply_svdm_nak(pd_port);
		return;
	}
#endif
...
extern void set_dp_notify_node(int val);
#endif
...
void pd_dpm_dfp_inform_id(struct pd_port *pd_port, bool ack)
{
...
#ifdef CONFIG_LGE_DP_UNSUPPORT_NOTIFY
		if (PD_IDH_PTYPE(payload[0]) & IDH_PTYPE_AMA) {
			DPM_DBG("Alternative Mode Adapter detected !\n");
			set_dp_notify_node(1);
		}
#endif
	}
	...
	if (!pd_port->pe_data.vdm_discard_retry_flag) {
		 ...
		 */
		if (!ack) {
			DPM_DBG("dpm_reaction_clear - DISCOVER_SVID !\n");
#ifndef CONFIG_LGE_USB // For Ellisys PD compliance TD.PD.VDMD.E1 tVDMSenderResponse Deadline
...
{
#if DPM_DBG_ENABLE || defined(CONFIG_LGE_USB)
	uint8_t ops = dpm_vdm_get_ops(pd_port);
#endif
