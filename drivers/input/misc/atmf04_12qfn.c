...
static void chg_mode(unsigned char flag, struct i2c_client *client)
{
	...
	if(flag == ON) {
		...
		if(data->platform_data->chip_version){// ALMF04
...
static unsigned char chk_done_erase(unsigned int wait_cnt, struct i2c_client *client)
{
	unsigned int trycnt = 0;
	unsigned char rtn;
	u32 FLAG_DONE_ERASE =0;
	struct atmf04_data *data = i2c_get_clientdata(client);
	if(data->platform_data->chip_version)//ALMF04
		FLAG_DONE_ERASE = 0x03;
	else 
		FLAG_DONE_ERASE = 0x02;
...
static unsigned char load_firmware(struct atmf04_data *data, struct i2c_client *client, const char *name)
{
  ...
  //      release_firmware(fw);
		...
		PINFO("AddTMF04 Firmware update is done");
	    }
	else {
...
#endif
		/* add */
		{"Adsemicon,chip_version",   &pdata->chip_version,    DT_SUGGESTED,   DT_U32,     0},
		{NULL,				NULL,				0,		0,		0},
	};
...
#endif
#ifdef CONFIG_MACH_MSM8996_TF10
	if(lge_get_board_subrev_no() > HW_SUB_REV_1)
		memcpy(InitCodeVal, InitCodeVal_rev12, sizeof(InitCodeVal));
#endif
...
{
#ifdef CONFIG_MACH_MSM8996_TF10
	if(lge_get_board_subrev_no() > HW_SUB_REV_2){
		PERR("revno enum [%d], skip init", lge_get_board_subrev_no());
		return 0;
	}
#endif
