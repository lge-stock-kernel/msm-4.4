...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	if (selected) {
		...
		if (check_memory_allocation_pressure(min_score_adj) == PRESSURE_HIGH) {
			...
			lowmem_print(3, "Memory Allocation is High\n");
			goto hswap_kill;
		}

		if (!reclaimable_cnt && (min_score_adj > OOM_SCORE_CACHED_APP_MIN_ADJ)) {
			rcu_read_unlock();
			rem = SHRINK_STOP;
			goto end_lmk;
		}
...
#else
		lowmem_print(1, "Killing '%s' (%d) (tgid %d), adj %hd, reclaimable cnt %d, top (%ld, %d), reason %d\n"
#endif
			        "   to free %ldkB on behalf of '%s' (%d) because\n" \
			        "   cache %ldkB is below limit %ldkB for oom_score_adj %hd\n" \
				"   Free memory is %ldkB above reserved.\n" \
