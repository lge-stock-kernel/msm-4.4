...
static struct fusb252_instance* workaround_avoiding_mbg_fault_singleton(void) {
	...
	if (IS_ERR_OR_NULL(workaround_amf_instance)) {
		...
		if (IS_ERR_OR_NULL(workaround_amf_instance) && _lge_adc_chip) {
			workaround_amf_instance
				= devm_fusb252_instance_register(_lge_adc_chip->dev, &workaround_amf_description);
			if (IS_ERR_OR_NULL(workaround_amf_instance))
				devm_fusb252_instance_unregister(_lge_adc_chip->dev, workaround_amf_instance);
		}
...
#ifdef CONFIG_LGE_USB_SWITCH_FUSB252
				workaround_avoiding_mbg_fault_usbid(true);
				rc = qpnp_vadc_read(lge_adc_chip->pm_vadc,
						lge_adc_chip->usb_id_channel, &results);
				workaround_avoiding_mbg_fault_usbid(false);
#else
				rc = qpnp_vadc_read(lge_adc_chip->pm_vadc,
					lge_adc_chip->usb_id_channel, &results);
#endif
